...
and checkExp  (ftab : FunTable)
    ...
    *)
    | Times (e1, e2, pos) ->                                   (* task 1: multiplication *)
        let (e1_dec, e2_dec) = checkBinOp ftab vtab (pos, Int, e1, e2)
        (Int, Times (e1_dec, e2_dec, pos))

    | Divide (e1, e2, pos) ->                                  (* task 1: division *)
        let (e1_dec, e2_dec) = checkBinOp ftab vtab (pos, Int, e1, e2)
        (Int, Divide (e1_dec, e2_dec, pos))

    | And (e1, e2, pos) ->                                     (* task 1: logical and *)
        let (e1_dec, e2_dec) = checkBinOp ftab vtab (pos, Bool, e1, e2)
        (Bool, And (e1_dec, e2_dec, pos))

    | Or (e1, e2, pos) ->                                      (* task 1: logical or *)
        let (e1_dec, e2_dec) = checkBinOp ftab vtab (pos, Bool, e1, e2)
        (Bool, Or (e1_dec, e2_dec, pos))

    | Not (e, pos) ->                                                      (* task 1: logical negation *)
        let (t, e') = checkExp ftab vtab e
        if t <> Bool then reportTypeWrong "argument of not" Bool t pos
        (Bool, Not (e', pos))

    | Negate (e, pos) ->                                                   (* task 1: numerical negation *)
        let (t, e') = checkExp ftab vtab e
        if t <> Int then reportTypeWrong "argument of negate" Int t pos
        (Int, Negate (e', pos))
